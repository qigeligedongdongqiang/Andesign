...
- (BOOL)jq_createTable:(NSString *)tableName dicOrModel:(id)parameters primaryKey:(NSString *)primaryKey
{
    return [self jq_createTable:tableName dicOrModel:parameters primaryKey:primaryKey excludeName:nil];
}

- (BOOL)jq_createTable:(NSString *)tableName dicOrModel:(id)parameters primaryKey:(NSString *)primaryKey excludeName:(NSArray *)nameArr
{
    
    NSString *sqlStr;
    if ([parameters isKindOfClass:[NSDictionary class]]) {
        sqlStr = [self createTable:tableName dictionary:parameters primaryKey:primaryKey excludeName:nameArr];
    } else {
        Class CLS;
        ...
        sqlStr = [self createTable:tableName model:CLS primaryKey:primaryKey excludeName:nameArr];
    }
    BOOL creatFlag;
...
- (NSString *)createTable:(NSString *)tableName dictionary:(NSDictionary *)dic primaryKey:(NSString *)primaryKey excludeName:(NSArray *)nameArr
{
    ...
    for (NSString *key in dic) {
        ...
        if ([key isEqualToString:primaryKey]) {
            if (keyCount == dic.count) {
                [fieldStr appendFormat:@" %@ %@ primary key)", key, dic[key]];
                break;
            }
            [fieldStr appendFormat:@" %@ %@ primary key,", key, dic[key]];
        } else {
...
- (NSString *)createTable:(NSString *)tableName model:(Class)cls primaryKey:(NSString *)primaryKey excludeName:(NSArray *)nameArr
{
    NSMutableString *fieldStr = [[NSMutableString alloc] initWithFormat:@"CREATE TABLE %@ (", tableName];
